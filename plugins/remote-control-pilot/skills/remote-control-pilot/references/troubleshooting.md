...
| Session "Needs input" on claude.ai, or a `permission prompt <Tool>:` line in `get_run_log` with no `tool_result` after it | Pending permission prompt (write outside the working directory, sensitive command) | Approve from claude.ai / the app / the terminal; "Always allow" for a series of files; anticipate with `/add-dir` or an `allow` rule on the target |
...
| `Artifact read` shows only the beginning of the page; the last files are missing | Large page: the tool output is truncated, the full HTML is saved to a local file | `Read` the saved file with offset/limit; keep base64 blobs at the end of the page or out of it (one such line ≈ 28k tokens) |
